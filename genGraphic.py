...
def taylor(x):
	return -1*(x-1)**0+-90*(x-1)**1+-1305/2*(x-1)**2+94680/6*(x-1)**3+2.76656e+06/24*(x-1)**4+-1.4919e+08/120*(x-1)**5
def y_prime(x):
	return np.cos(15*x**3+7)*15*3*x**2+np.sin(9*x+6)*-1*9*5*np.cos(9*x+6)**4
def tangent(x):
	return np.sin(15*1**3+7)+np.cos(9*1+6)**5+(np.cos(15*1**3+7)*15*3*1**2+np.sin(9*1+6)*-1*9*5*np.cos(9*1+6)**4)*(x-1)
