...
static curAnchorNode readNode(differentiator_t* differentiator, char* buffer, size_t* curBufferPos){
    ...
    if((buffer[*curBufferPos] == '(') ){
        LPRINTF("скобочка\n");
        createdNode = differentiatorCreateNode(differentiator, buffer, curBufferPos);
    }
    else{
...
static curAnchorNode differentiatorCreateNode(differentiator_t* differentiator, char* buffer, size_t* curBufferPose){
    assert(differentiator);
    assert(buffer);
